...
\chapter{About the notes: Roadmap}
...
A small description of important terms:
\begin{itemize}
	\item \textbf{LWE}: An LWE ciphertext encrypts an integer. It is a collection of two elements (a, b), with a being a vector of n elements, sampled randomly from a discrete uniform distribution, and b being an integer. b = $ a\cdot s $ + e, where s is a vector and is called a secret key, while e is an integer called the noise and is sampled from a discrete normal distribution. This is discussed in detail more in chapter 1.
	\item \textbf{RLWE}: RLWE encrypts a polynomial. Like LWE, it is also a collection of (a, b), but this time, they both are polynomials. The relationship between a and b is the same, but the noise and secret key are now polynomials instead of being an integer or a vector. This is discussed in detail in chatper 2.
	\item \textbf{Homomorphic NAND}: The homomorphic NAND operation is the NAND operation applied on two LWE ciphertexts, which encrypt boolean True or False. Since LWE ciphertexts encrypt integers, the boolean values  are represented as 2 (True) and 0 (False). The NAND operation is evaluated by first passing the two inputs through a function that performs a linear transform to get a single LWE output, which is then fed into a bootstrapping operation to evaluate the NAND operation. This is discussed in detail in chapter 5.
\end{itemize}
The second section is just chapter 6. It was written based on the \hyperref[ref:handbook]{TFHE-rs handbook}. This chapter goes specifically into TFHE, and describes the different kinds of ciphertexts and operations in TFHE. It has a fair bit of overlap (actually a lot of overlap) with the first section, as almost all the operations are the same and implemented more or less, similarly, except that TFHE uses the more 'general' GLWE, GLev and GGSW ciphertexts which while more complex, follow the same rules and steps for almost everything.\newline
\newline
...
\begin{equation}
	...
	\end{cases}
\end{equation}
This chapter provides a hands on example with implementation to explain how to create the TLU polynomial and then use it to evaluate a function on an LWE ciphertext using bootstrapping. It also talks about how the domain changes if the function to be evaluated is not negacyclic and why. Finally, it talks a little about bivariate PBS, that utilizes a packing trick to pack two ciphertexts into 1 so that PBS can be implemented to evaluate functions that take two inputs while traditional PBS only takes one LWE ciphertext. This is also done with an example of showing how to use this packing trick to implement PBS to evaluate the max operation between two ciphertexts.\newline

\textbf{IMPORTANT THING TO NOTE:} There might be places where I denoted a ring as $R_q(x)/ (x^N + 1)$. This is a typo, theyre supposed to be $Z_q(x)/ (x^N + 1)$ as the coefficients are integers.

\section{Important Abbreviations/Notations}
\begin{itemize}
	\item LWE: Learning With Errors Ciphertext/Problem
	\item RLWE: Ring Learning With Errors Ciphertext/Problem
	\item  PBS: Programmable Bootstrapping
	\item $\mathbb{Z}_q/(X^N + 1)$: A polynomial ring, where the polynomials have a max degree of N-1 and the coefficients are modulus q, $q \in \mathbb{Z}_+$. 
	\item Cleartext: Not an abbreviation but important. A regular number before being processed/encrypted is called cleartext.
	\item Plaintext: Not an abbreviation but important. A plaintext is the encoded cleartext.
	\item  TLU/LUT: A Lookup Table. Used for bootstrapping.
\end{itemize}
...
\chapter{PBS Implementation In Depth}
Please note that this chapter might contain some redundancy as I discuss the points already discussed in the previous chapters, however, here I try to go into a little more detail and implement some more PBS operations.\\
Programmable Bootstrapping, as studied above, does two things:
\begin{itemize}
	\item Reduces/refreshes noise in a polynomial.
	\item Evaluates a function (in TFHE).
\end{itemize}
However, there are things that should be taken care of when PBS is being implemented for a specific function. The most important thing that needs to be remembered is that it takes as input an LWE ciphertext and outputs an LWE ciphertext too, and is traditionally, a univariate function. For bootstrapping, you need something called as the \textbf{Redundant Lookup Table} is very important.

\section{Redundant Lookup Table/TLU}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\linewidth]{figures/TLU}
	\caption{Image showing transformation and redundancy. Taken from \href{https://www.zama.org/post/tfhe-deep-dive-part-4}{TFHE-Deep Dive, part 4}}
	\label{fig:tlu}
\end{figure}
The collection of cell blocks that correspond to the same value are called mega-cases. Here, one thing to notice is that the case for 0 is split, with half of it being on either ends. This is because at m = 0, the noise can put it on either side of the number line, positive noise moves it left while negative noise moves it right.\footnote{Assuming that its modeled as unsigned integer and the first half is used for positive while the second half is used for negative integers, as is the case in TFHE-rs.} \newpage

\subsection{Now why is this important?}
...
\section{Negacyclic Property and PBS}
Here, we talk about the usable domains and how does that get affected by the negacyclic property of the ring.\\
Remember that, a negacyclic ring $\mathbb{Z}_q/(X^N + 1)$ has an order of 2N, meaning, $X^N \equiv -1$ and $X^{2N }\equiv 1$. And therefore, for any polynomial from this ring, you need to rotate it, by 2N to get the original polynomial back. Also, rotating a polynomial $f(x)$ by N, gives you its negation $-f(x)$. Therefore, the actual domain of the rotation input becomes 2N and not N. You dont want to push further because after that it just loops around. However, the latter half of the rotation outputs is just the negation of the former half (think: rotate(i, F) = - rotate(i+N, f)), and you cant change that.\\
...
\section{Sign Operation Using PBS}
...
\subsubsection*{T: The TLU polynomial}
Consider a polynomial:
\begin{center}
	$\mathbf{T} = 0 + 0x + 0x^2 + ... + 0x^{N/2 - 1} - x^{N/2} - x^{N/2 + 1} - ... - x^{N-1}$
\end{center}
Try rotating this polynomial:\\
\begin{center}
	$\text{Rotate}(1, T) = 1 + 0x + ... + 0x^{N/2} - x^{N/2 - 1} - ... - x^{N-1}$\\
	$\text{Rotate}(N/2, T) = 1 + x + ... + x^{N/2 -1 } - 0x^{N/2} - ... - 0x^{N-1}$\\
	$\text{Rotate}(N, T) = 0 + 0x + ... + 0x^{N/2 -1 } + x^{N/2} + ... + 1x^{N-1}$\\
	$\text{Rotate}(-1, T) = 0 + 0x + 0x^2 + ... + 0x^{N/2 - 2} - x^{N/2 - 1} - x^{N/2} - ... - x^{N-2} - 0x^{N-1}$
\end{center}
Now for each of the rotate results, if you extract the first coefficient:
\begin{center}
	$\text{ExtractSample}(0, \text{Rotate}(1, T)) = 1$\\
	$\text{ExtractSample}(0, \text{Rotate}(N/2, T)) = 1$\\
	$\text{ExtractSample}(0, \text{Rotate}(N, T)) = 0$\\
	$\text{ExtractSample}(0, \text{Rotate}(-1, T)) = 0$\\
	$\text{ExtractSample}(0, \text{Rotate}(0, T)) = 0$
\end{center}
This can be generalized as:
\begin{equation}
	\text{ExtractSample}(0, \text{Rotate}(i, T)) = 
	\begin{cases}
		1 & \text{if} \ 0 < i \leq N/2\\
		0 & \text{if} \ -N/2 < i \leq 0
	\end{cases}
\end{equation}
...
\textbf{A few examples} (N = 1024, p = 16, q = $2^{32}$):\\
\begin{itemize}
	\item Consider you want to find the sign of x = 3. On encoding and scheme switching, you get:
	\begin{center}
	...
	\item Consider x = -2. On encoding and switching, you get:
	\begin{center}
...
\end{minted}
\newpage
\section{Bivariate PBS: Max using PBS}
While PBS traditionally only takes a single LWE ciphertext as input, there might be situations where you want to evaluate a PBS on two LWE ciphertexts, for example, calculating min, max etc, where it might not be trivial to reduce them into a single ciphertext. In these situations, you can use a \textit{packing trick}\footnote{Please be aware that I got this packing trick from claude, which mentions that this is what is being used in bivariate PBS in TFHE-rs (bivariate PBS functions exist in higher level APIs), but I haven't checked the source code to confirm this, but the method seems to be working.} to implement this.
\subsubsection{What is this packing trick?}
We want to somehow convert the two LWE ciphertexts into a single LWE ciphertext, such that both the ciphertexts are intact in this new \textit{packed} polynomial. One way to do this is the following:
\begin{itemize}
	\item You have the regular parameters p, q, and n for LWE.
	\item You create another parameter, lets called it $p_b$, such that $p_b$ < 2p. It should be small enough such that atleast two $p_b$s can fit in p. This $p_b$ is what actually defines the range of the cleartext now.
	\item Encode and encrypt the cleartexts using the actual plaintext modulus.
	\item Pack them using the following relationship. For two LWE ciphertexts, a and b:
	\begin{center}
		$p = a \cdot p_b + b$
	\end{center}
	\item For creating the LUT \footnote{In the previous section, we created the LUT polynomial ourselves, but this one is non-trivial, and thus, we will use TFHE-rs's built in accumulator, which takes logic in plaintext and then creates the LUT polynomial dynamically. This is only used for creating the LUT and not when working with ciphertexts, and thus, it can use operations such as divide etc.}, unpack the ciphertexts, by doing
	\begin{center}
		$a = p / p_b$\\
		$b = p \ \% \ p_b$
	\end{center}
...
let p_1 = 1u64 << 4; // this will actually define the available range for the clear messages. The idea is that the 2 LWE ciphertexts will be packed into one and then in the PBS, they will be extracted to allow for bivariate PBS. This allows for operations such as max, min etc.
let delta = (1u64 << 63) / p;

let msg1_clear = 7i8; // Im using 8 bit right now because with this noise level it works well. I will move up if needed but for now, im using 8 bit as it anyway is working for our model.
let msg2_clear = -3i8;
...
let max_accumulator: GlweCiphertextOwned<u64> = generate_programmable_bootstrap_glwe_lut(
	...
	|x: u64| { // this is 128 because p is 2^8 = 256, and the latter half represents negative values
		...
		let a = x / (p_1 as u64);
		let b = x % (p_1 as u64);
		let sa: i64 = if a < p_1 / 2 { a as i64 } else { a as i64 - p_1 as i64 }; // taking mod centered around 0 wrt p_1 to ensure that we get the correct values as they were encoded that way.
...
\chapter{Max from a TFHE encrypted array}
In this chapter, Im exploring ways to find max from an array of LWE ciphertexts, to use for the TRF model project. I will be trying to find ways to find argmax/max out of an array. This will include implementing relevant research papers, using sorting algorithms etc, and comparing their speeds.
\section{A non-comparison oblivious sort and its application to private	k-NN}
This \hyperref[ref:paper_comp_sort]{paper} implements a sorting algorithm for sorting a list of LWE ciphertexts that are 'packed' into an RLWE ciphertext by an operation called as \textbf{P}ublic \textbf{F}unctional \textbf{K}ey \textbf{S}witching (\textbf{PFKS}). Additioanlly, it also extends it by using the developed sorting algorithm to applications in k Nearest Neighbors by finding the k smallest elements from a list of LWE ciphertexts. For the sorting algorithm, the paper uses something called as \textbf{Counting sort}, which is a sorting algorithm that is not based on the methods that use comparisons. The benefit of this, alongside it not using comparisons is that instead of the worst case O(n log n) complexity, it has a worst case complexity of O(n + k) (n: input size; k: range of values). The authors of this paper have provided the code for the algorithm they propose in \hyperref[ref:paper_comp_sort-code]{this} github repository.
\begin{figure} [H]
	\centering
	\includegraphics[width=0.5\linewidth]{counting_sort_algo}
	\caption{Image taken from \hyperref[ref:paper_comp_sort]{the paper}}
	\label{fig:countingsortalgo}
\end{figure}\newpage
...
def counting_sort(a, rng):
	'''
	This function implements the counting sort. It takes as input the array a (which is assumed to only have positive values), and a rng (range), which is the 'length-of-the-range' from where the values in a come from.
	'''
	...
	return R
\end{minted}
\subsection{Blind Counting Sort}
...
\subsubsection{Blind Array Access (BAA)}
Blind Array Access allows one to access a coeffcient at a specific position in the RLWE ciphertext. This is done the same way it was being done in PBS, you first rotate the ciphertext such that the coefficient that you want is in the front, and then use the SampleExtract method to extract the 0th coefficent. This is the Blind reading operation.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{counting_sort_BAA}
	\caption{Image taken from \hyperref[ref:paper_comp_sort]{the paper}}
	\label{fig:countingsortbaa}
\end{figure}

\subsubsection{Blind Array Add (BAAdd)}
...
\begin{figure} [H]
	\centering
	\includegraphics[width=0.5\linewidth]{counting_sort_BAAdd}
	\caption{Image taken from \hyperref[ref:paper_comp_sort]{the paper}}
	\label{fig:countingsortbaadd}
\end{figure}
The blind counting sort then uses the operations defined above to then sort the coefficients of the RLWE ciphertext.  It basically just mimics the cleartext algortihm except that it uses the respective blind methods.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{blind_counting_sort}
	\caption{Image taken from \hyperref[ref:paper_comp_sort]{the paper}.}
	\label{fig:blindcountingsort}
\end{figure}

\subsection{Blind Top-k}
...
\subsection{How to use this paper to find max}
...
\begin{itemize}
	\item The maximum value was chosen among 16 values.
	\item The code was run on local machine (16GB RAM, R9 5900HS).
	\item No rayon based parallelization was used in either of the methods (except the ones that TFHE does internally).
	\item  The max operation was executed 100 times with each method and the runtimes for each run were recorded.
\end{itemize}

The following approaches were compared:
\begin{itemize}
	\item \textbf{O($n^2$) max:} Each element was compared with each other to find the maximum element. This approach is referred to as the \textit{TFHE-native} approach as it uses the native tfhe-rs methods available to calculate the maximum.
	\item \textbf{Couting Sort based max: } The paper's blind counting sort is used to find the max from the same array. This approach is referred to as the \textit{paper based} approach.
\end{itemize}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{figures/max-runtime-plot}
	\caption{Runtime Comparison Boxplot}
...
\begin{displaymath}
	\begin{array}{|c|c|}
		\hline
		\text{Approach} & \text{Mean Runtime (sec)} \\ \hline
		\text{TFHE-rs Native} & 4.12 \\ \hline
		\text{Counting Sort Based} & 1.35  \\ \hline
	\end{array}
\end{displaymath}

\section{Current Status}
\begin{itemize}
	\item I am now able to run both the blind sort and blind top-k algorithms. [DONE]
	\item I need to compare the runtime of the current approach (iteratively finding max) and the runtime of this algorithm for finding max. [DONE]
	\item Ensure that the 'trick' to make it work with negative numbers works. [In Progress]
\end{itemize}
...
\begin{itemize}
  ...
  \item \href{https://github.com/zama-ai/tfhe-rs-handbook}{The TFHE-rs Handbook.(Read until the PBS section, it describes a lot of other stuff too.)} \label{ref:handbook}
   ...
   \item \href{https://eprint.iacr.org/2024/1894.pdf}{A non-comparison oblivious sort and its application to private k-NN} \label{ref:paper_comp_sort}
   \item \href{https://github.com/sofianeazogagh/revoLUT}{CODE: A non-comparison oblivious sort and its application to private k-NN} \label{ref:paper_comp_sort-code}
   \item \href{https://docs.zama.org/concrete}{Concrete Library} \label{ref:concrete-library}
   \item \href{https://github.com/malb/lattice-estimator}{Lattice-estimator} \label{ref:lattice-estimator}
\end{itemize}
...
\chapter{How does concrete work? Why are the keysizes so large?}
\section{How does concrete work?}
\hyperref[ref:concrete-library]{Concrete}, just like tfhe-rs, is a library (but in python) created by ZAMA for TFHE computations. However, it is a much higher level library that decides a lot of things for you, so you only need to focus on the actual TFHE computation. The concrete-python library is merely a frontend/API for the original concrete compiler, which was developed by zama. The compiler does all the calculation, however, it accepts input in \href{https://mlir.llvm.org/}{MLIR}. To make it easier to work with, the python library was created. The library takes python code, which is then converted to MLIR codes and using the codes, the compiler creates the FHE circuits which finally perform the computation.
\begin{figure} [H]
	\centering
	\includegraphics[width=0.7\linewidth]{figures/concrete-architecture}
	\caption{Concrete Architetcute. Taken from \href{https://www.zama.org/post/the-architecture-of-concrete-zama-fully-homomorphic-encryption-compiler-leveraging-mlir}{this} blog by ZAMA.}
	\label{fig:concrete-architecture}
\end{figure}

\subsection{Parameter Selection}
One of the most important things to do when working with FHE (TFHE) is selecting the parameters such as $\sigma$, N, p, q, etc. These parameter values determine a lot of very important things, such as ;
\begin{itemize}
	\item Security Level of the encryption.
	\item Runtimes of arithmetic and PBS operations.
\end{itemize}
Here, unlike other libraries, the parameter selection is done automatically by the library. The parameters are selected such that they are secure against all possible attacks, and maintain a specific security level (for example, 128 bit). To do so, the \hyperref[ref:lattice-estimator]{lattice-estimator} tool is used, aiming for a specific security level $\lambda$.\newline\newline
Workflow of parameter selection:
\begin{itemize}
	\item Data Acquisition: For a given value $(n, q = 2^{64}, \sigma)$, raw data is obtained from lattice estimator and the corresponding security level $\lambda$. Then, change the value of $\sigma$ until the desired security level $\lambda_{target}$ is obtained. This is done for several values of n.
	\item Model Training for a given  $\lambda_{target}$: Since we now have many sets of $(n, q=2^{64}, \sigma)$ for the specified level, a model is fit on this raw data to find $\sigma$ as a function of n.
	\item Model Verification: Before using the parameters given by a model, they are verified with lattice estimator to ensure that they achieve the required security level before proceeding with them.
\end{itemize}

\section{Bit-width, Multi precsion and Multi parameters}
The bit-witdh (or the plaintext modulus), is another important parameter. The bit-width has a big impact on the runtime of any circuit, with lower values having faster runtimes and higher values having relatively lower runtimes. This too, however, is chosen by the library based on a few things and settings. Imagine a circuit implementing and evaluating $x^2 + y$, with them having the following ranges:
\begin{figure} [H]
	\centering
	\includegraphics[width=1.0\linewidth]{figures/circuit-example}
	\caption{Graph/circuit representation of $x^2+y$. Taken from \href{https://docs.zama.org/concrete/compilation/key-related-options-for-faster-execution/multi_precision}{concrete documentation}.}
	\label{fig:circuit-example}
\end{figure}
Turning \textbf{multi-precision} on, allows the library to use the lowest possible bit-widths for inputs of each operation in the above (or any, for that matter) circuit.  Which results in the following bit-widths:
\begin{figure} [H]
	\centering
	\includegraphics[width=1.0\linewidth]{figures/circuit-example-final-bitwidths}
	\caption{Final bit widths of the above graph.}
	\label{fig:circuit-example-final-bitwidths}
\end{figure}
The benefit of using the smallest possible bit-width is that it decreases runtimes as you are only using the smallest possible bit-width for an operation. This is where \textbf{multi-parameter} comes in. Since each operation is (potentially) using different parameters, you need to have different bootstrapping keys for each (wherever bootstrapping is being used), and also, to change an encrypted ciphertext from one parameter set to another, you also need keyswitch keys. Turning multi-parameter on allows the libary to do this. However, these keys are relatively large, and since there will be many such keys (especially for larger circuits), these sizes stack up, resulting in sometimes huge overall keysizes (in the order of tens of GBs). You can turn multi-parameter off, which could potentially reduce the overall sizes of the keys since there is just one key now. The bit-width used will be the one corresponding to the operation that has the largest range. Additionally, this can potentially increase the runtime as well.
\begin{figure}[H]
	\centering
	\begin{subfigure}[b]{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figures/multi-parameter-on}
		\caption{Multi Parameter: ON}
		\label{fig:multi-parameter-on}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figures/multi-parameter-off}
		\caption{Multi Parameter: OFF}
		\label{fig:multi-parameter-off}
	\end{subfigure}
	\caption{Effects of multi-parameter value on keysizes.}
	\label{fig:multi-parameter-combined}
\end{figure}

\chapter{TFHE-rs and Concrete-ML MLP Runtime}
\section{TFHE-rs}
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\linewidth]{figures/tfhe-rs-mlp-runtime}
	\caption{TFHE-rs runtime for MLP}
	\label{fig:tfhe-rs-mlp-runtime}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{figures/layer-runtime-pie}
	\caption{Pie-chart for layer runtimes}
	\label{fig:layer-runtime-pie}
\end{figure}

\section{Concrete-ml Runtime}
\begin{displaymath}
	\begin{array}{|c|c|c|}
		\hline
		\text{weight bit-width} & \text{identity bit-width} & \text{inference time/datapoint (in sec)}\\ \hline
		3 & 3 & 2,67 \\ \hline
		4 & 4 & 37.71 \\ \hline
		5 & 5 & 60.61 \\ \hline
	\end{array}
